...
function Navbar() {
  ...
  return (
    <>
      <nav className="bg-neutral-800 bg-opacity-5 fixed w-full z-20 top-0 start-0">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-5">
          ...
          <div
            ...
            id="navbar-sticky"
          >
            <ul className="flex flex-col text-lg p-4 md:p-0 mt-4 font-medium border border-neutral-700 rounded-lg bg-neutral-800 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-transparent">
              {navItems.map((item) => (
                <li key={item.id}>
                  <a
                    href={item.href}
                    className={`
                      ...
                      ${pathname === item.href ? "bg-neutral-900" : "hover:bg-neutral-900"}
                    `}
                    aria-current="page"
                  >
                    {item.text}
